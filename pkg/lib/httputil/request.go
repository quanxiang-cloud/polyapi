...
func httpRequest(reqURL, method, data string, header http.Header, owner string, client *http.Client) (string, *http.Response, error) {
	...
	if err != nil {
		putErrLog(err, "httpRequest.NewRequest")
		return "", nil, err
	}
	req.Header = header
	...
	if err != nil {
		putErrLog(err, "httpRequest.Do")
		return "{}", nil, err
	}

	respBody, err := readBody(resp)
	if err != nil || resp.StatusCode != http.StatusOK {
		putErrLog(err, "httpRequest.ReadResp:"+resp.Status)
	}
...
func readBody(resp *http.Response) ([]byte, error) {
	if resp.Body == nil {
		return defaultBody(resp, nil), nil
	}
	body, err := io.ReadAll(resp.Body)
	resp.Body.Close()
	return defaultBody(resp, body), err
}

func defaultBody(resp *http.Response, body []byte) []byte {
	if resp.StatusCode != http.StatusOK {
		resp.Status = fmt.Sprintf("proxy:%s", resp.Status)
	}
	if len(body) == 0 {
		if resp.StatusCode != http.StatusOK {
			body = []byte(fmt.Sprintf(`{"status":"%s"}`, resp.Status))
		} else {
			body = []byte("{}")
		}
	}
